...
// 해상도에 따른 최적화된 설정 반환
GstPipelineConfig gst_get_optimized_config_for_resolution(int width, int height) {
    GstPipelineConfig config = DEFAULT_PIPELINE_CONFIG;
    
    // 해상도별 최적화
    if (width >= 1920 && height >= 1080) {
        // 1080p 이상 - 고품질 설정
        config.jpeg_quality = 90;
        config.h264_bitrate = 8000000;
        config.framerate = 30;
        config.interpolation_method = 5;
        config.preset_level = 2; // medium
    } else if (width >= 1280 && height >= 720) {
        // 720p - 균형잡힌 설정
        config.jpeg_quality = 85;
        config.h264_bitrate = 4000000;
        config.framerate = 30;
        config.interpolation_method = 4;
        config.preset_level = 1; // ultrafast
    } else {
        // 저해상도 - 속도 우선
        config.jpeg_quality = 75;
        config.h264_bitrate = 2000000;
        config.framerate = 30;
        config.interpolation_method = 3;
        config.preset_level = 0; // ultrafast
    }
...
// 성능 모니터링 함수
void gst_ros_publisher_get_stats(GstRosPublisher *publisher, 
                                unsigned long *frame_count,
                                unsigned long *bytes_processed) {
    if (!publisher) return;
    
    pthread_mutex_lock(&publisher->mutex);
    if (frame_count) *frame_count = publisher->frame_count;
    if (bytes_processed) *bytes_processed = publisher->bytes_processed;
    pthread_mutex_unlock(&publisher->mutex);
}

void gst_ros_publisher_reset_stats(GstRosPublisher *publisher) {
    if (!publisher) return;
    
    pthread_mutex_lock(&publisher->mutex);
    publisher->frame_count = 0;
    publisher->bytes_processed = 0;
    pthread_mutex_unlock(&publisher->mutex);
}

// 설정 업데이트 함수
// v4l2src 기반에서는 config 업데이트가 불필요하므로 제거됨
int gst_ros_publisher_update_config(GstRosPublisher *publisher, 
                                   const GstPipelineConfig *config) {
    // v4l2src 기반에서는 설정 변경이 불필요
    (void)publisher;
    (void)config;
    return 0;
}
...
// v4l2src 기반에서는 create_with_config 함수가 불필요하므로 제거됨
/*
GstRosPublisher* gst_ros_publisher_create_with_config(int camera_id, int width, int height, 
                                                     GstEncodingType encoding_type,
                                                     const GstPipelineConfig *config,
                                                     rcl_node_t *node) {
    GstRosPublisher *publisher;
    char pipeline_str[4096];  // 더 큰 버퍼로 변경
    char topic_name[64];
    char camera_info_topic[80];
    char h264_topic[80];
    ...
    // 파이프라인 문자열 생성
    if (encoding_type == GST_ENCODING_JPEG) {
        snprintf(pipeline_str, sizeof(pipeline_str), GST_PIPELINE_TEMPLATE_JPEG_OPTIMIZED, width, height);
    } else if (encoding_type == GST_ENCODING_H264) {
        snprintf(pipeline_str, sizeof(pipeline_str), GST_PIPELINE_TEMPLATE_H264_OPTIMIZED, width, height);
    } else if (encoding_type == GST_ENCODING_RAW) {
        snprintf(pipeline_str, sizeof(pipeline_str), GST_PIPELINE_TEMPLATE_RAW_OPTIMIZED, width, height);
    } else if (encoding_type == GST_ENCODING_RGB) {
        snprintf(pipeline_str, sizeof(pipeline_str), GST_PIPELINE_TEMPLATE_RGB_OPTIMIZED, width, height);
    } else if (encoding_type == GST_ENCODING_UYVY) {
        snprintf(pipeline_str, sizeof(pipeline_str), GST_PIPELINE_TEMPLATE_UYVY_SIMPLE, width, height);
    } else if (encoding_type == GST_ENCODING_MULTI_STREAM) {
        snprintf(pipeline_str, sizeof(pipeline_str), GST_PIPELINE_TEMPLATE_MULTI_STREAM, width, height);
    // econ 권장 v4l2src 기반 파이프라인들
    } else if (encoding_type == GST_ENCODING_V4L2_RGB) {
        snprintf(pipeline_str, sizeof(pipeline_str), GST_PIPELINE_TEMPLATE_ECON_V4L2_RGB, 
                 camera_id, width, height, width, height);
    } else if (encoding_type == GST_ENCODING_V4L2_I420) {
        snprintf(pipeline_str, sizeof(pipeline_str), GST_PIPELINE_TEMPLATE_ECON_V4L2_I420, 
    ...
    // v4l2src 기반인 경우 source는 필요 없음 (appsrc와 달리 설정할 것이 없음)
    // appsrc 기반인 경우에만 source 가져오기
    bool needs_appsrc = (encoding_type != GST_ENCODING_V4L2_RGB && 
                         encoding_type != GST_ENCODING_V4L2_I420 && 
                         encoding_type != GST_ENCODING_V4L2_UYVY);
    
    if (needs_appsrc) {
        publisher->source = gst_bin_get_by_name(GST_BIN(publisher->pipeline), "source");
        if (!publisher->source) {
            printf("Failed to get appsrc element\n");
            if (publisher->sink) gst_object_unref(publisher->sink);
            gst_object_unref(publisher->pipeline);
            pthread_mutex_destroy(&publisher->mutex);
            free(publisher);
            return NULL;
    ...
    // 멀티 스트림의 경우 추가 sink 가져오기
    if (encoding_type == GST_ENCODING_MULTI_STREAM) {
        publisher->sink_h264 = gst_bin_get_by_name(GST_BIN(publisher->pipeline), "sink-h264");
        if (!publisher->sink_h264) {
            printf("Failed to get H.264 sink element\n");
        }
    }
    
    if (!publisher->sink) {
        printf("Failed to get sink element\n");
        if (publisher->source) gst_object_unref(publisher->source);
        if (publisher->sink_h264) gst_object_unref(publisher->sink_h264);
        gst_object_unref(publisher->pipeline);
        pthread_mutex_destroy(&publisher->mutex);
        free(publisher);
        return NULL;
    ...
    // 퍼블리시 초기화
    publisher->image_publisher = (rcl_publisher_t*)malloc(sizeof(rcl_publisher_t));
    publisher->camera_info_publisher = (rcl_publisher_t*)malloc(sizeof(rcl_publisher_t));
    
    // 멀티 스트림인 경우 H.264 퍼블리시도 생성
    if (encoding_type == GST_ENCODING_MULTI_STREAM) {
        publisher->h264_publisher = (rcl_publisher_t*)malloc(sizeof(rcl_publisher_t));
        snprintf(h264_topic, sizeof(h264_topic), "/dev/video%d/h264", camera_id);
    }
    
    if (!publisher->image_publisher || !publisher->camera_info_publisher || 
        (encoding_type == GST_ENCODING_MULTI_STREAM && !publisher->h264_publisher)) {
        printf("Failed to allocate publishers\n");
        if (publisher->image_publisher) free(publisher->image_publisher);
        if (publisher->camera_info_publisher) free(publisher->camera_info_publisher);
        if (publisher->h264_publisher) free(publisher->h264_publisher);
        gst_object_unref(publisher->source);
        gst_object_unref(publisher->sink);
        if (publisher->sink_h264) gst_object_unref(publisher->sink_h264);
        gst_object_unref(publisher->pipeline);
        pthread_mutex_destroy(&publisher->mutex);
    ...
    // 메모리 초기화 (매우 중요!)
    memset(publisher->image_publisher, 0, sizeof(rcl_publisher_t));
    memset(publisher->camera_info_publisher, 0, sizeof(rcl_publisher_t));
    if (publisher->h264_publisher) {
        memset(publisher->h264_publisher, 0, sizeof(rcl_publisher_t));
    }
    
    // 이미지 퍼블리시 초기화
    const rosidl_message_type_support_t *image_type_support = 
        ROSIDL_GET_MSG_TYPE_SUPPORT(sensor_msgs, msg, Image);
    
    rcl_publisher_options_t publisher_options = rcl_publisher_get_default_options();
    publisher_options.qos = rmw_qos_profile_sensor_data;
    
    rcl_ret_t ret = rcl_publisher_init(publisher->image_publisher, node, 
                                       image_type_support, topic_name, 
                                       &publisher_options);
    if (ret != RCL_RET_OK) {
        printf("Failed to initialize image publisher: %s\n", rcl_get_error_string().str);
        free(publisher->image_publisher);
        free(publisher->camera_info_publisher);
        gst_object_unref(publisher->source);
        gst_object_unref(publisher->sink);
        gst_object_unref(publisher->pipeline);
        pthread_mutex_destroy(&publisher->mutex);
    ...
    // H.264 퍼블리시 초기화 (멀티 스트림인 경우)
    if (encoding_type == GST_ENCODING_MULTI_STREAM && publisher->h264_publisher) {
        rcl_publisher_options_t h264_options = rcl_publisher_get_default_options();
        h264_options.qos = rmw_qos_profile_sensor_data;
        
        ret = rcl_publisher_init(publisher->h264_publisher, node, 
                                 image_type_support, h264_topic, 
                                 &h264_options);
        if (ret != RCL_RET_OK) {
            printf("Failed to initialize H.264 publisher: %s\n", rcl_get_error_string().str);
            rcl_publisher_fini(publisher->image_publisher, node);
            rcl_publisher_fini(publisher->camera_info_publisher, node);
            free(publisher->image_publisher);
            free(publisher->camera_info_publisher);
            free(publisher->h264_publisher);
            gst_object_unref(publisher->source);
            gst_object_unref(publisher->sink);
            if (publisher->sink_h264) gst_object_unref(publisher->sink_h264);
            gst_object_unref(publisher->pipeline);
        ...
        printf("H.264 publisher initialized for topic: %s\n", h264_topic);
    }
    ...
    // 메인 루프 생성
    publisher->loop = g_main_loop_new(NULL, FALSE);
