...
MosaicViewNode::MosaicViewNode() : Node("mosaic_view_node")
{
...
void MosaicViewNode::initializeVPIImages()
{
...
void MosaicViewNode::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr& front_msg,
                  ...
                  const sensor_msgs::msg::Image::ConstSharedPtr& right_msg)
{
    try {
        ...
        if (front_msg->width != static_cast<uint32_t>(camera_width_) || front_msg->height != static_cast<uint32_t>(camera_height_)) {
...
void MosaicViewNode::uploadToVPI(const cv::Mat& cv_image, VPIImage vpi_image)
{
...
void MosaicViewNode::downloadFromVPI(VPIImage vpi_image, cv::Mat& cv_image)
{
    ...
    if (first_call) {
        RCLCPP_INFO(this->get_logger(), "VPI image info: %dx%d, format: %ld", vpi_width, vpi_height, static_cast<long>(vpi_format));
        first_call = false;
    }
...
void MosaicViewNode::processSurroundView()
{
...
void MosaicViewNode::compositeMosaicView()
{
    static bool first_call = true;
...
void MosaicViewNode::CHECK_STATUS(VPIStatus status)
{
